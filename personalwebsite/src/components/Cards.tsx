    ...
    return (
        <a className="max-w-max min-w-full grid grid-rows-1 grid-cols-[120px_1fr] auto-rows-auto auto-cols-auto gap-5 h-[10%] min-h-[10%] cursor-pointer hover:bg-[#f3f5fd] border-2 border-slate-100 rounded-lg" href={href} target="_blank">
            ...
            <div className="flex flex-row flex-wrap">
                <div className="w-[75%] justify-self-start place-self-start grid grid-rows-2 grid-cols-1 auto-rows-auto auto-cols-auto text-sm">
                ...
                <h2 className="flex-1 md:text-right not-md:text-left whitespace-nowrap justify-self-end place-self-start pr-1 text-sm">{optional}</h2>
            </div>
        </a>
    )
}
...
export function Largecard({ img, title, description, href } : LargeCardProps) {
    return (
        <a className="max-w-max min-w-full gap-5 mb-5 flex-col place-self-center h-[400px] min-h-[20%] cursor-pointer hover:bg-[#f3f5fd] border-2 border-gray-100 rounded-md" href={href} target="_blank">
