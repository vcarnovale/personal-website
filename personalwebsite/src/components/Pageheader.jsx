...
export function Dropmen({ dropbuttons, drophref }) {
    return (
        <div className="sm:hidden absolute bg-black left-0 top-15 w-full">
...
function ShowButtons({ menubuttons, menuhref }) {
    return (
        <div>
            <nav class="hidden sm:block space-x-15 text-lg pr-15 text-white font-semibold" aria-label="main">
...
export default function Pageheader({ title, href, buttons }) {
    ...
    return (
        ...
        <div className="bg-black max-w-full mx-auto p-4 flex justify-between items-center sticky">
            ...
            <div>
                ...
                <button id="mobile-open-button" className="sm:hidden focus:outline-none text-2xl cursor-pointer text-white" onClick={() => setOpen(!open)}>&#9776;</button>
                {
                    (() => {
                        if (open) {
